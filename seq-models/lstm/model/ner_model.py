    ...
    def run_evaluate_2(self, test):
        ...
        for keys in pico_metrics:
            for annotype in annotypes:
                pico_metrics[keys][annotype] = 0.
            
        accs = []
        p_accs, i_accs, o_accs = [], [], []
        correct_preds, total_correct, total_preds = 0., 0., 0.
        for words, labels in minibatches(test, self.config.batch_size):
            labels_pred, sequence_lengths = self.predict_batch(words)
            ...
            for lab, lab_pred, length in zip(labels, labels_pred,
                ...
                i_lab_pred_chunks = set([item for item in lab_pred_chunks if item[0] == 'I'])
                o_lab_chunks = set([item for item in lab_chunks if item[0] == 'O'])
                o_lab_pred_chunks = set([item for item in lab_pred_chunks if item[0] == 'O'])

                correct_preds += len(lab_chunks & lab_pred_chunks)
                total_preds   += len(lab_pred_chunks)
                total_correct += len(lab_chunks)

                p_accs += [a==b for (a, b) in zip(lab, lab_pred) if a == 1]
