...
class PICOAbstractReader:
    ...
    def _load_docs(self):
        fn_train = [DATA_PATH + "{0}_abstract_train.data".format(self._annotype)]
        self.train_texts, self.train_labels, self.train_ids = self._load_docs_from_file(fn_train)

        fn_dev = [DATA_PATH + "{0}_abstract_dev_train.data".format(self._annotype)]
        self.dev_texts, self.dev_labels, self.dev_ids = self._load_docs_from_file(fn_dev)

        fn_test = [
                DATA_PATH + "{0}_abstract_dev_gt.data".format(self._annotype),
                DATA_PATH + "{0}_abstract_test_gt.data".format(self._annotype)
                ]
        self.test_texts, self.test_labels, self.test_ids = self._load_docs_from_file(fn_test)


    def _load_docs_from_file(self, filenames, tokenize=True):
        texts = []
        labels = []
        ids = []
